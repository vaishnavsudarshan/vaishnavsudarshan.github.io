...
Valency is how many arguments a verb takes. An *argument* is basically an $NP$ that is mandatory within a version of a verb. "I ate" is ambitransitive. It can either have a valency of $1$, which makes a verb where only the subject matters. Or it can have a valency of $2$, where in that version of the verb, something has to be eaten too. 

It gets confusing because you might think anything that can have a case for it is an argument. However, it depends on what's actually necessary for that version of the verb. For "I ate in the house", the "ate" will never care about *where* it's being eaten - not in any version of the verb. 
