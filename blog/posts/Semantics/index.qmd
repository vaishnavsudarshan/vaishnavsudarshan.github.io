...
tags: [Logic, Psychology, Philosophy, Neuroscience]
image: not_hot1.png
...
Still, from the perspective of describing a language's grammar, why would you need semantics? Is syntax not enough to explain how a language works? Nope! Consider this famous counterexample by [Noam Chomsky](https://en.wikipedia.org/wiki/Noam_Chomsky):
...
%% Styling
style S fill:#ffcccc,stroke:#333,stroke-width:2px,font-size:18px
style NP fill:#ccffcc,stroke:#333,stroke-width:2px,font-size:18px
style NP1 fill:#ccffcc,stroke:#333,stroke-width:2px,font-size:18px
style NP2 fill:#ccffcc,stroke:#333,stroke-width:2px,font-size:18px
style VP fill:#ccccff,stroke:#333,stroke-width:2px,font-size:18px
style V fill:#ccccff,stroke:#333,stroke-width:2px,font-size:18px
style A fill:#fff3b0,stroke:#333,stroke-width:2px,font-size:18px
style A1 fill:#fff3b0,stroke:#333,stroke-width:2px,font-size:18px
style A2 fill:#f9c0ff,stroke:#333,stroke-width:2px,font-size:18px
style C fill:#ffffff,stroke:#333,stroke-width:2px,font-size:18px
style G fill:#ffffff,stroke:#333,stroke-width:2px,font-size:18px
style I fill:#ffffff,stroke:#333,stroke-width:2px,font-size:18px
style S1 fill:#ffffff,stroke:#333,stroke-width:2px,font-size:18px
style F fill:#ffffff,stroke:#333,stroke-width:2px,font-size:18px

```

This sentence is of the form $S \to \text{Adj Adj N V Adv}$. This is syntactically valid. Consider the sentence "big golden dogs run quickly". This is also of the form $S \to \text{Adj Adj N V Adv}$. 

However, the former is ungrammatical to most sane English speakers, while the latter is perfectly fine. For one, "colorless" and "green" logically contradict each other, since green is a color. Also, when we look at the rest of the sentence, how can an idea be green? Or sleep? And that too, sleep *furiously*? Most people would only use physical attributes on concrete things, unless they're being poetic or metaphorical. Without any such specification, however, it definitely seems to be nonsensical. Therefore, a language's grammar must also include the logic behind meanings. 
...
One sense can also point to multiple references - for example, if you think of a "person", then that reference points to over 8 billion people. You could potentially solve this problem by saying that the set of all people is still a single set. 
...
Once, I watched [this short](https://www.youtube.com/shorts/hRl2B8YMyA8) by Karl Rock, a travel vlogger from New Zealand who explores India and Pakistan. Notice how he says, "Like 'Chinese', there's no language called 'Indian'". The semantic ambiguity from the negation is kind of weird here. 

It could either mean that:

* "Indian" is similar to "Chinese" in that the entire country doesn't speak a single language named after it (which is a plausible interpretation since some linguists prefer to call China's dominant language Mandarin instead, and Chinese refers to the entire Sinitic branch of the Sino-Tibetan language family, including distinct but related languages like Cantonese). This comes from the "like" comparing "Chinese" to "there is no language called Indian"

* There is no language called "Indian" in the same way that there is a language called "Chinese" (which means he's using Chinese to solely refer to Mandarin, which is also very common). This comes from the "like" comparing "Chinese" to just "Indian". See how this one doesn't contain the "no", leading to opposite interpretations?

The funny part is that it doesn't even matter which interpretation it was! The point is that there is no language called "Indian", but either way of determining to scope of "like" to fundamentally name Chinese as the family or the language gets you the same idea across. 
