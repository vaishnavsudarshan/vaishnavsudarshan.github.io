...
[Syntax](https://vaishnavs.net/posts/Syntax%20Part%201/) might allow you to arrange these meanings in the correct way, but compositional semantics allows you to get the overall idea as to what the sentence is actually saying. If you understand each smaller part, then you will understand the whole sentence, even if you haven't seen this specific syntactical structure before. 
...
Also, if you're going to use a concept very often, and it's tedious to describe it using many words, it can either shorten or take a loanword. 
...
However, these cases don't take away from the main point that meanings as a whole are derived from smaller parts - that's how you understand this sentence. It's just that sometimes, meanings do become new smaller parts.  
...
"Every student passed" would be written as ∀x (student(x) -> passed(x)). Basically, x can be anything, and if it is a student, then it passed, due to the universal quantifier. It specifically represents the set of all such x. This should hopefully illustrate how formal semantics works without a bunch of yapping.
