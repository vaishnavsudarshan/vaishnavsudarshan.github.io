...
This is an interesting case of semantic ambiguity with time. I'm gonna use a bit of math (to foreshadow formal semantics, which is later) to formalize this, although you subconsciously get the joke because it's common sense. Basically, let $t_0$ be the time right now, $A(T)$ be Arthur's age at a moment in time, $D(T)$ to be D.W.'s age at a moment in time, and $t$ be their age gap. Their mom means $A(t_0) = D(t_0+t)$, but D.W. thinks it means $A(T) = D(T)$ for some $T$ which is impossible because $A(T) - D(T) = t \neq 0$ for all $T$. 
...
* [Crash Course Linguistics Episode 5: Semantics]()
