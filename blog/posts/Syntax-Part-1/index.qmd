---
title: "Syntax - Recursion, Constituent Units, and Trees"
author: "Vaishnav Sudarshan"
date: "2026-6-10"
categories:  [Linguistics, CS]
...
You may have learned about syntax in English class in school, and parts of what you likely learned are pretty close to linguistics, like phrases and stuff, while others are horrendously off. 

Syntax is basically sentence structure. However, if someone says, "this sentence has short syntax" when referring to a short or simple sentence, that statement sounds reasonable, but it's like saying "this rock is intelligent", as in it's kinda nonsensical. Just say "this sentence is short" or something. To avoid confusion about what syntax actually is, it's probably better to define syntax as the set of rules that govern how a language allows for sentences to be formed (syntax describes a language, not an individual sentence).

# Bracketing and Constituents

Take any complicated sentence in English. For example, take the sentence "The student who the professor whom the dean hired praised left." This seems so confusing at first. However, we see that the basic sentence is just "the student left". We can put brackets around "left", as well as everything before "left", that just boils down to the student: "[The student who the professor whom the dean hired praised] [left]". The "who the professor who the dean hired praised" describes the student, and the "whom the dean hired" just describes the professor, so we can put brackets around each of those: "[The student [who the professor [whom the dean hired] praised]] [left]". 
...
One way to tell the role that a constituent plays is to replace it with another unit whose role we know. For example, "[The student [who the professor whom the dean hired praised]]" can be replaced with "[The student]", which we know represents the subject. Therefore, every single wrd before "left" is a constituent that represents the subject. Similarly, "left" is a constituent that represents the verb. If you want, you can replace "left" with "ate ice cream", because this constituent represents what the student *did*.

Constituents help us figure out what words can be put together. In English, subjects and objects are *noun phrases*. Pronouns, plurals, and proper nouns can be their own noun phrases, while common nouns need a determiner. So, "Canada" is its own noun phrase, while "a country" is also a noun phrase that is the same syntactic unit. You can say "I go to [Canada]" or "I go to [a country]" but you can't say "I go to country" or "I go to a Canada". 

## Head-Initial and Head-Final Order

Languages can be syntactically classified under whether the beginning or end of a phrase determines what type of constituent it behaves like. In English, "ate the cake" is a verb phrase (VP), which can be written as V NP (verb noun phrase). The category of "ate the cake" is the same as "ate", not "the cake". Additionally, "to the house" is a prepositional phrase (PP), and it can be decomposed as P NP. Therefore, we call English a head-initial language. 

Tamil, on the other hand, is a head-final language. The equivalent VP to "ate the cake" would be "cake sappitten", and that last word means "(I) ate", which is a verb, so the phrase can be decomposed as NP V.  Tamil also uses postpositions. The postpositional phrase "to the house" would be "vi:tu ku". Since "ku" means "to" and "vi:tu" means "house", this is decomposed as NP P, the latter of which determines the constituency. 
...
There are a finite amount of rules for how to generate a valid sentence, but an infinite amount of possible sentences (partially thanks to recursion, as you are about to see). You can represent the structure of possible sentences using trees. Note that only the last layer of a tree actually says what each word is. So, "fruit flies like a banana" and "hungry people eat that food" would have their trees looking about the same. Because a language is so complex, it's hard to write down every specific structure of sentences, so you can only be pretty general.

# Scope Ambiguity

Obviously, in spoken language, you don't have brackets or syntax trees. So, how would you interpret this sentence?

"The dog and the cat that you own fought each other".

Either this means that you own both the dog and the cat ("[[The dog and the cat] that you own] fought each other"), or your cat fought another dog ("[The dog and [the cat that you own]] fought each other").  

Another example is "I saw the man with a telescope", which either means the man has a telescope and you saw him, or you used a telescope to see the man.

In general, scope ambiguity is when you can't tell what constituent is being modified.

Prescriptivists are always like, "commas save lives, people" ("I love cooking my parents and my dog" vs "I love cooking, my parents, and my dog"). However, ambiguous sentences like the ones I showed before that are perfectly acceptable in standard English and the ambiguity can't just be fixed by adding commas (because that would be considered grammatically incorrect then since prescriptivists say that you can't put commas in those places). This is just yet another argument for descriptive linguistics - prescriptivists can't even be consistent with their own objectives, so it's better to just let language be. 
...
Now, look at this code snippet in Python:

```python
def reverse(s): # return a String that is the reverse of s
    if len(s) == 1: # base case: if the string has 1 character, 
        return s # it is inherently the reverse of itself, so we can just return it
    return s[-1] + reverse(s[:-1]) # move the last letter to the beginning and find the reverse of the rest
```
...
The connection between syntactic recursion and recursion in math is better shown when you look at the syntax tree. Two of the rules that English has to make noun phrases are $NP \to A \ \ NP$ and $NP \to N$.

Here is the "tree" for the word (and NP) "dogs":
...
classDef sentenceGroup fill:#ffe6cc,stroke:#ff8c00,stroke-width:3px,color:#000;
classDef nounGroup fill:#d9ecff,stroke:#1f77b4,stroke-width:3px,color:#000;
classDef verbGroup fill:#ddffdd,stroke:#2ca02c,stroke-width:3px,color:#000;
classDef modGroup fill:#fff2cc,stroke:#d4a017,stroke-width:3px,color:#000;

%% WORD CLASS (default purple)
classDef wordGroup fill:#e0e0ff,stroke:#6b6bbd,stroke-width:3px,color:#000;

class NP nounGroup;
class N nounGroup;

class dogs wordGroup;

```

Now, here's the tree for "brown dogs":
...
class brown wordGroup;
class dogs wordGroup;

```

Now, here's the tree for "big brown dogs":
...
Finally, here's the tree for "friendly big brown dogs":
...
Do you see how geometrically, this doesn't look that different from Serpenski's Triangle? 

In Serpenski's Triangle, you can zoom in to one of the corner triangles and it looks just like the overall triangle and you can do this over and over again (until you stop drawing, which is the base case). Note that I changed the base case to make a finite amount of zooming in (as opposed to zooming out) to make it more analogous to syntax trees. 

In these trees, you can decompose an NP into A NP, and zoom into the tree of that NP and do the same thing, until the NP just turns into a single noun, which is the base case. In theory, you can just stack infinite adjectives before one another (keeping in mind that English does have rules for what order adjectives can come in, though). 

# More Examples of English Recursion

English doesn't just need "of" (or equivalently, "'s") or stacking adjectives to be able to recurse. Using posessions or adjectives means a noun phrase is being a recursed. Here are some other examples of English being able to recurse on different constituents.
...
Now that we know how you can break apart sentences into smaller pieces, we can talk about the roles of the smaller pieces themselves! Stay tuned for morphosyntax, including one of the most confusing concepts in linguistics (good luck)! 
