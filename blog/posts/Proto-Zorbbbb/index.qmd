...
date: "2026-8-18"
categories:  [Conlangs, Linguistics]
tags: [Cryptography]
image: "example_script.png"
abstract: My showcase for Proto-Zorbbbb, the ancestor of Zorbbbb. Please don't try to speak it, because each verb has almost nine trillion forms, the number system is based on prime numbers rather than the powers of a base, you must have perfect pitch, and your mouth must be capable of manipulating 4D space. 
---
...
A lot of features that *should* exist aren't yet present. This is because Proto-Zorbbbb, is, well, a proto-language. Just wait for my blog post on Zorbbbb to see that evolution emerge. But unlike Proto-Indo-European, where there's no need to learn it because you might as well learn Vedic Sanskrit or Ancient Greek and not miss out on much, Proto-Zorbbbb is VERY different from Zorbbbb and barely recognizable as belonging to the same language family. 
...
Lemme clarify what this means. Discrete means the number of theoretically possible quantities for this noun is at most $\aleph_0$, and continuous means the number of theoretically possible quantities for this noun is $2^{\aleph_0}$. 

1D means the number belongs on the real number line, 2D for complex, 4D for quaternions, and beyond for, well, beyond. 
...
Liquids - write ⟨L⟩ or ⟨R⟩ right after the semivowel, which may be null
...
Also, monkeys have special declensions. The root for all monkeys is simply the root for the generic pronoun, which is "E g bbbb". For every pattern for monkeys, the bottom most pattern is moved to the top, and the right most is moved to the left, pushing everything else down or right. For example, here’s what the example for case:
...
(Monkeys do not fall under beasts). 
...
$n = \sum\limits_{i=0}^{k} C_i X_i$, where $C_i \in \{0, 1\}$ and $k$ is the number such that $X_k \leq n$ but $X_{k+1} > n$. 
...
The overall word order is VOS. Apart from the $VP \to V O$, everything else is head-final. There is no movement (just let it descend into Zorbbbb). 
...
At least transcribing it shall be easy, since none of the noun agreement stuff is even recorded.

![The same thing in the orthography](example_script.png){.lightbox}

<details>
<summary>How I did this while still being lazy (It's really not that deep)</summary>

Once I installed my font, Paint allowed me to type it with the text feature. 

I first copy and pasted the raw consonants into a Google Doc. I then cut each consonant to just the first letter. The dashes had served their visual purpose now, which was to show me where I should stop deleting, so I removed all of them right after. 

Then, to account for the allographs, I swapped the case of the first letter after each space (and remember that ? became ! because yes). The $0$ key is what maps to the glyph for word boundaries, so I swapped each space with a $0$. 

It was now time to put this into a text box in Paint, so I pasted what I had so far. To get a bunch of lines with blocks of $8$ characters, I just pressed enter to make a new line every $8$ characters (with a single character on the last line). 

But now I had to reverse every other line. To do this like a machine, I put a bar at the end of these lines. Then, I took the first character in the section before the bar and put it right after the bar, until the bar was at the beginning, at which point it had served its purpose, so buh-bye!

Coding this would definitely have allowed me to be even lazier (since the algorithm is simple), but it was almost midnight when I typed this and I needed a mind-numbing exercise to do to get sleepy anyway. At least it didn't feel like it was done manually in the sense that I could rely solely on muscle memory after the algorithm was perfected. 
...
* [The Art of Language Invention](https://www.goodreads.com/en/book/show/24611649-the-art-of-language-invention) is a great resource for learning about the components of a conlang, and most of it is applicable to just understanding linguistics concepts in general. 

* [Biblaridion's How to Make a Language series](https://www.youtube.com/watch?v=FHK1gO2Mh68&list=PL6xPxnYMQpqsooCDYtQQSiD2O3YO0b2nN) is another great resource to learn what things to consider while conlanging. 

* I was inspired by Arabic to use root-and-pattern morphology (read my article on [morphology](https://vaishnavs.net/posts/Morphology/) to learn about this) and also a cursive writing system. 

* The phenomenon of alternating the writing direction to not have to move your pen to the beginning of a line is caled [boustrophedon](https://en.wikipedia.org/wiki/Boustrophedon), common in inscriptions from Ancient Greece. 

* I sought a number system that doesn't use powers of bases, because there are a lot of ways to [partition](https://en.wikipedia.org/wiki/Integer_partition) an integer. The only main contraint was that there had to be a unique partition for each number.

* The decision to split the copula into "equals" and "belongs to" was based on the logical fallacy "I am an animal, and my cat is an animal, therefore I am my cat". The flaw here is obviously common sense, but most of the Proto-Zorbbbbistanis were NOT smart, so the prescriptivist intellectuals had to fix this by creating the distinction.
